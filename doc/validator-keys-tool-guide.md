...
change if the brtd config and/or server are compromised.
...
*   sign tokens authorizing a brtd server to run as the validator identified
    by this public key.
...
The current token needs to be present in the brtd config file.
...
```
  Update brtd.cfg file with these values:
...
For a new validator, add the [validator_token] value to the brtd config file.
...
value. After the config is updated, restart brtd.
...
```
  ...
  Update brtd.cfg file with these values and restart brtd:
...
restart brtd. Rename the old key file and generate new [validator keys](#validator-keys) and
